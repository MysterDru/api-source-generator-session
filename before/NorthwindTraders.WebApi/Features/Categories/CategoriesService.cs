...
public sealed class CategoriesService(ILogger<CategoriesService> logger, INorthwindRepository repository)
    : ICategoriesService
{
    private static Expression<Func<Category, CategorySummaryResponse>> CategoryProjection => c =>
    ...
    private static Expression<Func<Product, ProductSummaryResponse>> ProductProjection => p =>
    ...
    public async Task<CategorySummaryResponse> GetAsync(int id, CancellationToken cancellationToken)
    {
        ...
        if (result == null) throw new NotFoundException(nameof(Category), id);
